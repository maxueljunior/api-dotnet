...
public class PedidoController : MainController
{
    ...
    public async Task<IActionResult> UltimoPedido()
    {
        ...
        if (pedido is null)
        {
            AdicionaErroProcessamento("Pedido não encontrado!");
    ...
    private async Task<bool> ValidarCarrinhoProdutos(CarrinhoDTO carrinho, IEnumerable<ItemProdutoDTO> produtos)
    {
        if (carrinho.Itens.Count != produtos.Count())
        {
            ...
            foreach (var itemId in itensIndisponiveis)
            {
                ...
                AdicionaErroProcessamento($"O item {itemCarrinho.Nome} não está mais disponível no catálogo, o remova do carrinho para prosseguir com a compra");
            }
        ...
        foreach (var itemCarrinho in carrinho.Itens)
        {
            ...
            if (produtoCatalogo.Valor != itemCarrinho.Valor)
            {
                ...
                AdicionaErroProcessamento(msgErro);
                ...
                {
                    AdicionaErroProcessamento($"Não foi possível remover automaticamente o produto {itemCarrinho.Nome} do seu carrinho, _" +
                ...
                {
                    AdicionaErroProcessamento($"Não foi possível atualizar automaticamente o produto {itemCarrinho.Nome} do seu carrinho, _" +
                                               "adicione novamente caso ainda deseje comprar este item");
                    return false;
                }

                LimparErrosProcessamento();
                AdicionaErroProcessamento(msgErro + " Atualizamos o valor em seu carrinho, realize a conferência do pedido e se preferir remova o produto");
