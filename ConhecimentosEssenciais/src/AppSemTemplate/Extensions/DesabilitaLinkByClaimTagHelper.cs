...
public class DesabilitaLinkByClaimTagHelper : TagHelper
{
    ...
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var temAcesso = ClaimsAuthorization.ValidarClaimsUsuario(_contextAccessor.HttpContext, IdentityClaimName, IdentityClaimValue);

        if (temAcesso) return;

        output.Attributes.RemoveAll("href");
        output.SuppressOutput(); //Apaga tudo;
